...
import { Alert, Image, SafeAreaView, StyleSheet, Text, TouchableOpacity, View, FlatList } from "react-native";
...
const ListProducts = props => {
	...
	useEffect(() => {
		if (!!props.referenceScanned) {
			...
			if (product.length > 0) {
				...
				Alert.alert("Erreur système", "SecondLife ne peut pas trouver votre produit. Veuillez réessayer ou chercher dans la liste des produits");
			}
	...
	const renderItem = ({ item, type }) => {
		return (
			<RectButton style={[styles.item, { opacity: type === "sold" ? 0.4 : 1 }]} key={item.sku} onPress={() => handleSelectProduct(item)}>
				<View style={{ width: 80, height: 80 }}>
					<TouchableOpacity onPress={() => handleSelectProduct(item)} style={{ flex: 1 }}>
						<Image
							source={{
								uri: "data:image/png;base64," + item.product.pictures[0].content
							}}
				...
				<View style={{ paddingHorizontal: 20 }}>
					...
					<Text style={styles.productSubtitle}>
						{item.product.brand} - <Text style={{ fontWeight: "bold" }}>{item.product.price} €</Text>
					</Text>
					<Text style={styles.productSubtitle}>{`${item.customer.firstName} ${item.customer.lastName} - ${item.product.createdDate
						.split(" ")[0]
						.replace(/-/g, ".")}`}</Text>
				</View>
			</RectButton>
	...
	const renderNoItem = () => (
		<View style={{ flex: 3 }}>
			<Text style={{ textAlign: "center", fontWeight: "bold", fontSize: 24 }}>Il n'y a aucun produits</Text>
		</View>
	);
			...
			<SafeAreaView style={{ flex: 3 }}>
				<FlatList
					...
					renderItem={({ item, index }) => (
						<SwipeableComponent key={index} index={index} data={item} type="sold" handleUnsellProduct={handleUnsellProduct}>
	...
	return (
		<SafeAreaView
			...
			{showList === "haventsold" ? renderProductsHaventSold() : renderProductsSold()}
			<View style={{ marginBottom: 60 }}></View>
			<ModalProduct
				visible={productDetail.modal}
				product={productDetail.product}
				handleModifyProduct={() => props.handleModifyProduct(productDetail.product)}
				handleSellProduct={() => {
