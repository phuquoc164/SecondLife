...
const CameraScan = props => {
	...
	const handleReadQRCode = event => {
		...
		if (data.type && data.type === "product") {
			...
			FetchService.post("vouchers", data, props.token)
				...
				.catch(error => {
					console.debug(error);
					Alert.alert("Problème de système", "SecondLife a rencontré une problème. Veuillez re-scanner!");
				});
		} else {
	...
	const returnScanPage = () => (
		<View style={{ position: "relative", flex: 1 }}>
			...
			<QRCodeScanner
				...
				cameraStyle={{
					height: Platform.OS === "ios" ? Dimensions.get("screen").height - 60 : Dimensions.get("screen").height - 90
				}}
			/>
		</View>
	);
	...
	return resultPage.show ? (
		<ResultPage
			...
			title={resultPage.isSuccess ? "Ce bon d'achat a bien\nété validé" : "Ce bon d'achat a déjà\nété utilisé"}
			btnComponent={() => (
