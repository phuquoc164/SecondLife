...
const SwipeableComponent = (props) => {
  ...
  const renderActions = (progress) => (
    <View
      ...
      {props.type === 'sold' ? (
        renderAction('Marqué\nà vendre', colors.black, 80, progress, () =>
          props.handleUnsellProduct(props.data),
        )
      ) : (
        <>
          {renderAction('Marqué\nVendu', colors.green, 160, progress, () =>
            props.handleSellProduct(props.data),
          )}
          {renderAction("Modifier\nl'article", colors.black, 80, progress, () =>
            props.handleModifyProduct(props.data),
          )}
        </>
      )}
    </View>
