...
const PickerCategories = (props) => {
    ...
    const handleSelectData = (item) => {
        ...
        if (data.step === 1) {
            ...
            const selectedPicker =
                item.Name === data.selectedPicker.Name
                    ? getSelectedPicker(1)
                    : {Id: null, Name: null};
            setData({
    ...
    const renderListItem = (defaultSelected, item) => {
        if (data.step === 1 || data.step === 2) {
            return (
                <TouchableOpacity onPress={() => handleSelectData(item)}>
                    {renderListItemBase(defaultSelected, item)}
                </TouchableOpacity>
            )
        }
    ...
    const renderListItemBase = (defaultSelected, item) => (
        <View style={styles.styleListItem}>
            ...
            <View style={{flex: 1, flexDirection: 'row', alignItems: 'center'}}>
                {item.HasImage && (
                <View
                    style={{width: 32, alignItems: 'center', marginRight: 10}}>
                    <Image
    ...
    const renderSearch = (handleClose) => (
      <View
        ...
        <TouchableOpacity
          onPress={() => handleBackButtonPressed(handleClose)}
          style={{position: 'absolute', left: 15, top: 29}}>
          <Image
    ...
    return (
      <PickerModal
        renderSelectView={(disabled, selected, showModal) =>
          renderSelectView(showModal, data.dataSelected, "Sélectionnez une catégorie")
        }
